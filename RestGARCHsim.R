...
#mean_skewnorm = mean(rst(100000, xi= 0, omega = (13*pi)/(13*pi - 25), alpha = 5, nu=Inf))

#########################generate a large sample to compute the mean and variance of skew-t dist.
#rand = rst(100000, xi= 0, omega = (13*pi)/(13*pi - 25), alpha = 5, nu=5)
#aaa = mean(rand)
#mean_skewt = rep(aaa, nn)
#bbb = var(rand)
#var_skewt = rep(bbb, nn)
...
for(j in 1:k){
  ...
  #eta1 = rnorm(nn, -0.75, sqrt(10/16))
  #eta2 = rnorm(nn, 0.25, 1)
  #u = runif(nn)
  #eta = (u< 1/4)*eta1 + (u>= 1/4)*eta2
  
  ##########skew normal
  #eta = rst(nn, xi= 0, omega = (13*pi)/(13*pi - 25), alpha = 5, nu=Inf)
  #eta = eta - mean_skewnorm
  
  ##########skew t
  #eta = rst(nn, xi= 0, omega = (13*pi)/(13*pi - 25), alpha = 5, nu=5)
  #eta = as.vector(eta)
  #eta = (eta - mean_skewt)/sqrt(var_skewt)
